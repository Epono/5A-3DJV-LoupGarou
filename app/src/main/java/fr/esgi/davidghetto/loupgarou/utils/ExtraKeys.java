package fr.esgi.davidghetto.loupgarou.utils;

public class ExtraKeys {
    public static final String ROLES_LIST_KEY = "roles";
    public static final String PLAYERS_LIST_KEY = "players";

    public static final String PICK_ACTIVITY_KEY = "pick";
    public static final String PICK_ACTIVITY_HEADER = "pick_header";

    public static final String VOTE_ACTIVITY_KEY = "vote";
    public static final String VOTE_ACTIVITY_HEADER = "vote_header";

    public static final String DISPLAY_PLAYER_KEY = "display";
}
